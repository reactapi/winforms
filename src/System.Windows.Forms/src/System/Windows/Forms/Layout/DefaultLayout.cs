    ...
    internal partial class DefaultLayout : LayoutEngine
    {
        ...
        public static void ScaleAnchorInfo(IArrangedElement element, SizeF factor)
        {
            ...
            if (anchorInfo is not null)
            {
                ...
                if (UseAnchorLayoutV2(element))
                {
                    // AutoscaleFactor is not alligned with Window's SuggestedRectangle applied
                    // on top-level window/Form. So, compute factor with respect to the change
                    // in DisplayRectangle and apply it to scale anchors. See dotnet/winforms#8266
